...
class PauseMenu:
    ...
    def create_objects(self):
        ...
        new_game = Button(position, (100, 50), (240, 240, 240), "New game", font, lambda: self.__game.reset())
        self.__objects.append(new_game)

        position = (self.__game.screen_width / 2 - 50, self.__game.screen_height / 2 + 50)
        action = lambda: self.__game.visualizer.flip_use_visualizer()
        visualization = CheckBox(position, (25, 25), (240, 240, 240), "Visualization", font,
                                       False, action)
        self.__objects.append(visualization)

        position = (self.__game.screen_width / 2 - 50, self.__game.screen_height / 2 + 85)
        action = lambda: self.__ai_settings[0].flip_iterative_deepening()
        ...
        action = lambda: self.__ai_settings[0].flip_alpha_beta()
        ...
        action = lambda: self.__ai_settings[0].flip_order_moves()
